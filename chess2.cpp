...
static void Draw(const Board& b,WORD defAttr,bool hasSel,Pos sel,const vector<Pos>& marks){
...
static Board ApplyMove(const Board& b,const Move& m,ApplyInfo* info=nullptr){
	if(info) *info=ApplyInfo{};
	Board nb=b;
	...
	if(m.isCastle && p.t=='K'){
		...
		if(info){ info->didEnPassant=true; info->didCapture=true; }
		
		nb.s[m.to.r][m.to.c]=p;
		int capR=m.to.r+(us==WHITE?-1:+1);
		nb.s[capR][m.to.c]=Make(NONE,'.');
	}else{
		if(captured.c!=NONE && info) info->didCapture=true;
		
		nb.s[m.to.r][m.to.c]=p;
		
		if(m.promo && p.t=='P'){
			nb.s[m.to.r][m.to.c].t=m.promo;
			if(info) info->didPromotion=true;
		}
...
static void AddIfLegal(const Board& b,vector<Move>& out,const Move& m){
	Board nb=ApplyMove(b,m,nullptr);
...
static string Key(const Board& b){
	string k;
	k.reserve(1+128+6);
	k.push_back(char('0'+b.sideToMove));
	
	for(int r=0;r<8;r++){
		for(int c=0;c<8;c++){
			char col=(b.s[r][c].c==WHITE?'w':b.s[r][c].c==BLACK?'b':'.');
			k.push_back(col);
			k.push_back(b.s[r][c].t);
		}
	}
	
	k.push_back(b.cr.wk?'K':'-');
	k.push_back(b.cr.wq?'Q':'-');
	k.push_back(b.cr.bk?'k':'-');
	k.push_back(b.cr.bq?'q':'-');
	k.push_back(b.epAvail?char('A'+b.epTarget.r):'-');
	k.push_back(b.epAvail?char('a'+b.epTarget.c):'-');
	return k;
}

static bool ReadMove(Move& mv,Color side){
	string tok;
	cout<<"> ";
	if(!(cin>>tok)) return false;
	
	string up=tok;
	for(char& ch: up) ch=toupper((unsigned char)ch);
	
	if(up=="O-O" || up=="0-0"){
		mv.isCastle=true;
		mv.isEnPassant=false;
		mv.promo=0;
		mv.from=(side==WHITE?Pos{0,4}:Pos{7,4});
		mv.to=(side==WHITE?Pos{0,6}:Pos{7,6});
		return true;
	}
	if(up=="O-O-O" || up=="0-0-0"){
		mv.isCastle=true;
		mv.isEnPassant=false;
		mv.promo=0;
		mv.from=(side==WHITE?Pos{0,4}:Pos{7,4});
		mv.to=(side==WHITE?Pos{0,2}:Pos{7,2});
		return true;
	}
	
	int x1;
	try{ x1=stoi(tok); }catch(...){ return false; }
	int y1,x2,y2;
	if(!(cin>>y1>>x2>>y2)) return false;
...
static bool ReadMoveByClickOrKeyboard(const Board& b,const vector<Move>& legal,bool& hasSel,Pos& sel,Move& mv){
	HANDLE hIn=GetStdHandle(STD_INPUT_HANDLE);
	DWORD nEvents=0;
	...
	if(nEvents==0){
		hasSel=false;
		return ReadMove(mv,b.sideToMove);
	}
	...
	while(true){
		...
		if(rec.EventType==MOUSE_EVENT){
			...
			if(me.dwEventFlags==0 && (me.dwButtonState & FROM_LEFT_1ST_BUTTON_PRESSED)){
				Pos cell;
				if(!ConsoleCharToBoardCell(me.dwMousePosition.X,me.dwMousePosition.Y,cell)) continue;
				
				if(!hasSel){
					if(b.s[cell.r][cell.c].c!=b.sideToMove) continue;
					sel=cell;
					hasSel=true;
					return false;
				}else{
					...
					if(p.c==b.sideToMove && p.t=='P' && b.epAvail && mv.to.r==b.epTarget.r && mv.to.c==b.epTarget.c){
						if(abs(mv.to.c-mv.from.c)==1 && b.s[mv.to.r][mv.to.c].c==NONE) mv.isEnPassant=true;
					}
...
int main(){
	...
	cout<<"随机先手："<<(b.sideToMove==WHITE?"白方":"黑方")<<"\n";
	Sleep(800);
	...
	while(true){
		...
		vector<Pos> marks=hasSel?MarkTargets(legal,sel):vector<Pos>{};
		
		system("cls");
		Draw(b,defAttr,hasSel,sel,marks);
		...
		if(legal.empty()){
			if(inCheck){
				cout<<(b.sideToMove==WHITE?"白方":"黑方")<<"被将死，"<<(b.sideToMove==WHITE?"黑方":"白方")<<"胜利！\n";
			}else{
				cout<<"平局：逼和（无子可走且未被将军）。\n";
			}
			break;
		}
		
		if(inCheck) cout<<"提示：当前为将军状态，必须应将。\n";
		cout<<"鼠标左键：点起点再点终点；右键取消选择。也可键盘输入：x1 y1 x2 y2 或 O-O / O-O-O\n";
		
		Move mv;
		bool got=ReadMoveByClickOrKeyboard(b,legal,hasSel,sel,mv);
		if(!got){
			continue;
		}
		
		Piece p=b.s[mv.from.r][mv.from.c];
		if(!mv.isCastle && p.c==b.sideToMove && p.t=='P'){
			int promoRow=(b.sideToMove==WHITE?7:0);
			if(mv.to.r==promoRow){
				cout<<"兵到达底线，需要升变。输入 Q/R/B/N > ";
				char pr;
				if(!(cin>>pr)){
					cin.clear();
		...
		for(const auto& lm: legal){
			if(SameMove(lm,mv)){ ok=true; break; }
		}
		if(!ok){
			cout<<"非法走法，按回车继续...";
			cin.ignore(numeric_limits<streamsize>::max(),'\n');
		...
		ApplyInfo info;
		Board nb=ApplyMove(b,mv,&info);
		
		bool nextInCheck=InCheck(nb,nb.sideToMove);
		bool show=false;
		
		if(info.didCastle){ cout<<"提示：完成易位。\n"; show=true; }
		if(info.didEnPassant){ cout<<"提示：吃过路兵成功。\n"; show=true; }
		if(info.didPromotion){ cout<<"提示：完成升变。\n"; show=true; }
		if(nextInCheck){ cout<<"提示：将军！\n"; show=true; }
